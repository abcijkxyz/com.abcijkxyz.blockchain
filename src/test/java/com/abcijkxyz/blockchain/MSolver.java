...
public class MSolver {
	...
	static void calibrate() {
		...
		BoardPix = 0.5 * ((double) (lasw - firw) / (double) (BoardWidth - 2)) + 0.5 * ((double) (lash - firh) / (double) (BoardHeight - 2));
	...
	static int detect_3_7(int[] areapix) {
		...
		/*
		 * for(int i=0; i<15; i++){ for(int j=0; j<15; j++){ if(redx[i][j]) System.out.print("x"); else System.out.print(" "); } System.out.println(); } System.out.println();
		 */
		...
		// x .
		/*
		 * for(int i=0; i<14; i++){ for(int j=0; j<14; j++){ if(redx[i][j] && redx[i+1][j] && redx[i][j+1] && !redx[i+1][j+1]) return 3; } }
		 */
	...
	static int detect(BufferedImage bi, int i, int j) {
		...
		for (int rgb : areapix) {
			...
			if (blue > red && blue > green && colorDifference(red, green, blue, 220, 220, 255) < 200) {
				hasColorOfBlank = true;
			}
			if (colorDifference(red, green, blue, 167, 3, 5) < 20)
				return detect_3_7(areapix);
			if (colorDifference(red, green, blue, 29, 103, 4) < 20)
				return 2;
			if (colorDifference(red, green, blue, 0, 0, 138) < 20)
				return 4;
			if (colorDifference(red, green, blue, 124, 1, 3) < 20)
				return 5;
			if (colorDifference(red, green, blue, 7, 122, 131) < 20)
				return 6;
		}
	...
	static ArrayList<ArrayList<Pair>> tankSegregate(ArrayList<Pair> borderTiles) {
			...
			while (!queue.isEmpty()) {
				...
				for (Pair<Integer, Integer> tile : borderTiles) {
					...
					if (Math.abs(ci - ti) > 2 || Math.abs(cj - tj) > 2)
						...
						tilesearch: for (int i = 0; i < BoardHeight; i++) {
							for (int j = 0; j < BoardWidth; j++) {
								if (onScreen(i, j) > 0) {
									if (Math.abs(ci - i) <= 1 && Math.abs(cj - j) <= 1 && Math.abs(ti - i) <= 1 && Math.abs(tj - j) <= 1) {
										isConnected = true;
										break tilesearch;
									}
								}
	...
	/*
	 * todo - read / write calibration settings
	 * 
	 * Minor defects / bugs: 1) Calibration routine kind of sucks, can't calibrate at small resolutions and can't calibrate non-empty board 2) Death / win detection kind of sucks, can't distinguish
	 * win from loss and sometimes fails to detect either 3) Clicking order is highly non-human 4) Endgame solver is inefficient: we could make it kick in earlier if it was more efficient Known but
	 * non-fixable defects: 1) Cannot automatically detect number of mines
	 */
	...
	public static void main(String[] args) throws Throwable {
		...
		BoardHeight = 16;
		/*
		 * BoardPix = 35.267857; BoardTopW = 175; BoardTopH = 120;
		 */
		BoardPix = 42.035714;
		BoardTopW = 193;
		BoardTopH = 134;
		...
		/*
		 * updateOnScreen(); dumpPosition(); tankSolver(); exit();
		 */
